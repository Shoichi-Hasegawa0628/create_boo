...
class YoloObjectDetectorNode:
    ...
    def __init__(self):
        ...
        # ActionServer_1
        self._server_1 = actionlib.SimpleActionServer("/check_for_objects", yolo_ros_msgs.CheckForObjectsAction, self.action_call_back_1, auto_start=False)
        self._server_1.start()

        # ActionServer_2
        self._server_2 = actionlib.SimpleActionServer("/all_detection_data", yolo_ros_msgs.AllDetectionDataAction, self.action_call_back_2, auto_start=False)
        self._server_2.start()
    ...
    def main(self):
        while not rospy.is_shutdown():
            connection_exists = (self._detection_image_publisher.get_num_connections() > 0) or (self._bounding_boxes_publisher.get_num_connections() > 0)
    ...
    def action_call_back_1(self, goal):
        """
        Args:
        ...
        bounding_boxes = self._predict_image(image)
        if not self._server_1.is_preempt_requested():
            ...
            result.id = goal.id
            self._server_1.set_succeeded(result)
            rospy.loginfo("Return ActionResult")

    def action_call_back_2(self, goal):
        """
        Args:
            goal (yolo_ros_msgs.AllDetectionDataGoal):
        """
        rospy.loginfo("Receive ActionGoal")
        image = self._bridge.compressed_imgmsg_to_cv2(goal.image, desired_encoding="rgb8")
        bounding_boxes = self._predict_image(image)
        if not self._server_2.is_preempt_requested():
            result = yolo_ros_msgs.AllDetectionDataResult()
            result.bounding_boxes = self._generate_bounding_box_msg(bounding_boxes, self._classes)
            result.id = goal.id
            result.detect_image = self._bridge.cv2_to_compressed_imgmsg(self._generate_result_images(image, bounding_boxes, self._classes))
            self._server_2.set_succeeded(result)
            rospy.loginfo("Return ActionResult")
